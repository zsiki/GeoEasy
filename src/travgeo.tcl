...
#	headers
	...
	for {set i 0} {$i < $n} {incr i} {
		...
		GeoLog1  [format "           %10s %8s" [DMS $delta($i) sec] $t_1]
		if {$i > 0} {
			if {$beta($i) == ""} {
				...
				GeoLog1 [format "%-10s %10s %8.${decimals}f %8.${decimals}f %8.${decimals}f %10.${decimals}f %10.${decimals}f" \
					[lindex [lindex $stlist $i] 2] [DMS $beta($i) sec] $t($i) \
					$dx($i) $dy($i) [expr {$dx($i) + $vx($i)}] \
					[expr {$dy($i) + $vy($i)}]]
			}
		} else {
			if {$beta($i) == ""} {
				...
				GeoLog1 [format "%-10s %10s" \
					[lindex [lindex $stlist $i] 2] [DMS $beta($i) sec]]
			}
		}
		if {$i > 0} {
			if {$free} {
			...
			if {$beta(0) == "" || $beta($n1) == ""} {
				...
				GeoLog1 [format \
					...
					$pcode [DMS $vbeta($i) sec] $t_2 $vx($i) $vy($i) $x($i) $y($i)]
			}
		} else {
			if {$beta(0) == "" || $beta($n1) == ""} {
				GeoLog1 [format \
	...
	if {$beta(0) == "" || $beta($n1) == ""} {
		...
		GeoLog1 [format "           %10s                            %10.${decimals}f %10.${decimals}f" \
			[DMS 0] [expr {$x($n1) - $x(0)}] [expr {$y($n1) - $y(0)}]]
		GeoLog1 [format "           %10s %8.${decimals}f %8.${decimals}f %8.${decimals}f" \
			[DMS $sumbeta sec] $sumt $sumdx $sumdy]
		GeoLog1 [format "           %10s" \
			[DMS [expr {$n1 * $PI}]]]
		GeoLog1 [format "           %10s          %8.${decimals}f %8.${decimals}f" \
			[DMS $dbeta sec] $ddx $ddy]
	}
	if {! $free} {
